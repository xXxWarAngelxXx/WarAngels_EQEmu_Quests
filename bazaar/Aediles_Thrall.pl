...
my $secondbotcost  = 100;
my $thirdbotlevel  = 15;
my $thirdbotcost   = 150;
my $fourthbotlevel = 20;
my $fourthbotcost  = 200;
my $fifthbotlevel  = 25;
my $fifthbotcost   = 250;
...
sub EVENT_ITEM {
   if(defined $qglobals{bot_spawn_limit} && (quest::spawnbotcount() > $qglobals{bot_spawn_limit})) {
      ...
      if($success > 0) {
         ...
         $client->Message(6,"You can now create and spawn an Individual! See: '^help' or '^command help' and '^command usage' commands.");
         my $sbcount = quest::spawnbotcount();
