...
sub EVENT_SPAWN {
	...
	$npc->SetEntityVariable("wa_maxRange", 120); #Maximum range for the area the NPC will look to find a Mob to kill.
...
sub EVENT_TIMER {
	...
	if($timer eq "wa_Decide") { ###What do I want to do?####"eq" is used for strings not int "!" means NOT for numbers.
		plugin::wa_Decisions(30); #Chance to # <= KillMode or # > ChatMode. 50 is 50% kill and 50% chat. 30 is 30% chance to kill. 70% chance to chat.
